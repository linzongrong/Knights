...
class IndexTTS2:
    def __init__(
        ...
        self.bigvgan = bigvgan.BigVGAN.from_pretrained(bigvgan_name, use_cuda_kernel=True if self.use_cuda_kernel else False, local_files_only=True)
    ...
    def infer(self, spk_audio_prompt, text, output_path,
        ...
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt:
            # 使用多种方法加载音频
            try:
                # 方法 1: torchaudio (可能需要 soundfile)
                audio, sr = torchaudio.load(spk_audio_prompt)
                audio = audio.mean(dim=0)
            except Exception as e1:
                try:
                    # 方法 2: scipy.io.wavfile (适用于标准 WAV)
                    from scipy.io import wavfile
                    sr, audio_np = wavfile.read(spk_audio_prompt)
                    audio = torch.tensor(audio_np).float()
                    if audio.dim() > 1:
                    ...
                    try:
                        # 方法 3: 使用 wave 模块读取原始 PCM 数据
                        import wave
                        with wave.open(spk_audio_prompt, 'rb') as wf:
                            ...
                            if sampwidth == 2:
                                ...
                                audio = torch.from_numpy(audio_np).float() / 32768.0
                            elif sampwidth == 4:
                                audio_np = np.frombuffer(audio_data, dtype=np.int32)
                                audio = torch.from_numpy(audio_np).float() / 2147483648.0
                            else:
                        ...
                        # 方法 4: librosa (最后的选择)
                        print(f"所有方法都失败了，请安装 ffmpeg。错误：{e1}, {e2}, {e3}")
                        raise
        ...
        if self.cache_emo_cond is None or self.cache_emo_audio_prompt != emo_audio_prompt:
            # 使用多种方法加载音频，优先使用不需要 ffmpeg 的方法
            try:
                # 方法 1: torchaudio
                emo_audio_data, emo_sr = torchaudio.load(emo_audio_prompt)
                emo_audio_data = emo_audio_data.mean(dim=0)  # 转换为单声道
            except Exception as e1:
                try:
                    # 方法 2: scipy.io.wavfile
                    from scipy.io import wavfile
                    emo_sr, emo_audio_np = wavfile.read(emo_audio_prompt)
                    emo_audio_data = torch.tensor(emo_audio_np).float()
                    if emo_audio_data.dim() > 1:
                        emo_audio_data = emo_audio_data.mean(dim=1)
                    # 归一化到 [-1, 1]
                    if emo_audio_np.dtype == np.int16:
                        emo_audio_data = emo_audio_data / 32768.0
                    elif emo_audio_np.dtype == np.int32:
                        emo_audio_data = emo_audio_data / 2147483648.0
                except Exception as e2:
                    # 方法 3: librosa
                    print(f"torchaudio 和 scipy 都失败了：{e1}, {e2}，尝试使用 librosa...")
                    emo_audio, _ = librosa.load(emo_audio_prompt, sr=16000)
                    emo_audio_data = torch.tensor(emo_audio)
            
            if emo_sr != 16000:
                emo_audio_data = torchaudio.transforms.Resample(emo_sr, 16000)(emo_audio_data)
            emo_inputs = self.extract_features(emo_audio_data, sampling_rate=16000, return_tensors="pt")
